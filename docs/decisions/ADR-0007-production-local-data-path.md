...
- 状态：Phase 4 / Batch 1 实现完成，等待外部审阅
- 日期：2026-09-03
...
- Windows 优先使用 `%LOCALAPPDATA%\boss-job-radar\boss-job-radar.sqlite3`；`LOCALAPPDATA` 不存在时使用 `<home>\AppData\Local\boss-job-radar\boss-job-radar.sqlite3`。
