  ...
  it('persists the migration version and schema across file-backed reopen', async () => {
    ...
    try {
      ...
      expect(
        inspectionConnection
          .prepare(
            "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'job_opportunity_assessments'",
          )
          .get(),
      ).toEqual({ name: 'job_opportunity_assessments' });
      expect(
        inspectionConnection
          .prepare(
            "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'structured_llm_analyses'",
          )
          .get(),
      ).toEqual({ name: 'structured_llm_analyses' });
    } finally {
